...
class BaseModel(torch.nn.Module):
        ...
        for m in self.model:
            ...
            if x_aug is not None:
                if type(m).__name__ == 'FDM':
                    ...
                    # feature_visualization(F_inv_rgb, m.type, m.i, save_dir=Path("runs/detect/exp_inv_rgb"))
                    # feature_visualization(x_aug, m.type, m.i, save_dir=Path("runs/detect/exp_var_rgb"))
                    # feature_visualization(F_inv_rgb, m.type, m.i, save_dir=Path("runs/detect/exp_inv_inf"))
                    # feature_visualization(x_aug, m.type, m.i, save_dir=Path("runs/detect/exp_var_inf"))
                ...
                # feature_visualization(x_aug, m.type, m.i, save_dir=Path("runs/detect/exp_aug"))
    ...
    def loss(self, batch, preds=None, loss_feature_consistency_loss=None):
        ...
        if preds is None:   #在这个地方将进行完整的训练
            ...
            # batch["img"] = compose_image
            preds= self.predict(compose_image)
